...
/**
 * The Overview / Licenses tabs inside a product.
 *
...
export function ProductTabs({ productId }: { productId: string }) {
  ...
  const tabs = [
    { href: `/dashboard/products/${productId}`, label: "Overview", exact: true },
    {
  ...
  return (
    <nav className="-mb-px flex gap-1" aria-label="Product sections">
      {tabs.map((tab) => {
