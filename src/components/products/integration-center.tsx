...
/**
 * The four integration examples, with the developer's own product ID and
...
export function IntegrationCenter({
  ...
  const snippet = integrationSnippet(language, { productId, appUrl });
        ...
        >
          <div className="flex flex-wrap items-center justify-between gap-2">
            ...
            <CopyButton
              ...
              // reaches storage; nothing about the snippet or the product.
              onCopied={() => writeUiFlag(`copied-snippet:${productId}`, true)}
            />
          </div>
          ...
          {SNIPPET_LANGUAGES.map((candidate) => (
            <TabsContent key={candidate} value={candidate} className="mt-4">
              <pre className="max-h-96 overflow-auto rounded-lg border border-border bg-muted/40 p-4 text-xs leading-relaxed">
                <code>{integrationSnippet(candidate, { productId, appUrl })}</code>
              </pre>
            </TabsContent>
          ))}
        </Tabs>
