 ...
 */
async function seedCast(applicationId: string) {
...
async function run(applicationId: string, overrides: Partial<LicenseQuery> = {}) {
  return queryLicenses(db, DEVELOPER_A, applicationId, query(overrides), NOW);
}

describe("queryLicenses — ownership", () => {
  it("refuses an application owned by another developer", async () => {
    const application = await makeApplication(db, { ownerId: DEVELOPER_B });
    await makeLicense(db, { applicationId: application.id });

    await expect(
      queryLicenses(db, DEVELOPER_A, application.id, query(), NOW),
    ).rejects.toThrow(/not found/i);
  });

  it("reports a missing application identically to a foreign one", async () => {
    await expect(
      queryLicenses(db, DEVELOPER_A, "app_nothing", query(), NOW),
    ).rejects.toThrow(/not found/i);
  });

  it("never returns licenses belonging to another developer's application", async () => {
    const mine = await makeApplication(db, { ownerId: DEVELOPER_A });
    const theirs = await makeApplication(db, { ownerId: DEVELOPER_B });
    await makeLicense(db, { applicationId: mine.id, label: "Mine" });
    await makeLicense(db, { applicationId: theirs.id, label: "Theirs" });
...
describe("queryLicenses — effective status", () => {
  it("marks an active license past its expiry as expired", async () => {
    ...
    const cast = await seedCast(application.id);

    const result = await run(application.id);
...
describe("queryLicenses — status filter", () => {
  it("returns everything for 'all'", async () => {
    const application = await makeApplication(db, { ownerId: DEVELOPER_A });
    await seedCast(application.id);

    expect((await run(application.id, { status: "all" })).total).toBe(5);
  });
  ...
  it("excludes expired and revoked from 'active'", async () => {
    ...
    const result = await run(application.id, { status: "active" });
  ...
  it("returns only revoked for 'revoked'", async () => {
    ...
    const result = await run(application.id, { status: "revoked" });
    expect(result.total).toBe(1);
  ...
  it("returns only expired for 'expired'", async () => {
    ...
    const result = await run(application.id, { status: "expired" });
    expect(result.total).toBe(1);
...
describe("queryLicenses — activation filter", () => {
  it("returns only bound licenses for 'activated'", async () => {
    ...
    const result = await run(application.id, { activation: "activated" });
  ...
  it("returns only unbound licenses for 'unactivated'", async () => {
    ...
    const result = await run(application.id, { activation: "unactivated" });
    expect(result.total).toBe(3);
...
describe("queryLicenses — lock filter", () => {
  it("returns only device-locked licenses for 'locked'", async () => {
    ...
    const result = await run(application.id, { lock: "locked" });
    expect(result.total).toBe(3);
  ...
  it("returns only unlocked licenses for 'unlocked'", async () => {
    ...
    const result = await run(application.id, { lock: "unlocked" });
    expect(result.total).toBe(2);
    expect(result.rows.every((row) => !row.hwidLocked)).toBe(true);
  });
});
...
describe("queryLicenses — search", () => {
  it("matches a label case-insensitively", async () => {
    ...
    const result = await run(application.id, { q: "acme" });
    expect(result.total).toBe(1);
  ...
  it("matches notes", async () => {
    ...
    const result = await run(application.id, { q: "INVOICE" });
    expect(result.total).toBe(1);
    expect(result.rows[0]?.id).toBe(cast.permanent.id);
  });
  ...
  it("matches the last four characters of the key", async () => {
    ...
    const [row] = (await run(application.id)).rows;
    const result = await run(application.id, { q: row!.keyLast4.toLowerCase() });
  ...
  it("matches a substring rather than only a prefix", async () => {
    ...
    expect((await run(application.id, { q: "orp" })).total).toBe(1);
  });
  ...
  it("treats % as a literal character, not a wildcard", async () => {
    ...
    expect((await run(application.id, { q: "%" })).total).toBe(1);
    expect((await run(application.id, { q: "50% off" })).total).toBe(1);
  });

  it("treats _ as a literal character, not a single-character wildcard", async () => {
    const application = await makeApplication(db, { ownerId: DEVELOPER_A });
    await makeLicense(db, { applicationId: application.id, label: "order_9" });
    await makeLicense(db, { applicationId: application.id, label: "orderX9" });

    expect((await run(application.id, { q: "order_9" })).total).toBe(1);
  });

  it("treats a backslash as a literal character", async () => {
    const application = await makeApplication(db, { ownerId: DEVELOPER_A });
    await makeLicense(db, { applicationId: application.id, label: "path\\to\\seat" });
    await makeLicense(db, { applicationId: application.id, label: "unrelated" });

    expect((await run(application.id, { q: "path\\to" })).total).toBe(1);
  });
  ...
  it("ignores surrounding whitespace in the search term", async () => {
    ...
    expect((await run(application.id, { q: "  acme  " })).total).toBe(1);
  });
  ...
  it("returns everything for an empty search", async () => {
    ...
    expect((await run(application.id, { q: "" })).total).toBe(5);
    expect((await run(application.id, { q: "   " })).total).toBe(5);
  });
  ...
  it("does not match an unlabeled license on its empty label", async () => {
    ...
    const result = await run(application.id, { q: "Delta" });
    expect(result.total).toBe(1);
  });
});
...
describe("queryLicenses — combined filters", () => {
  it("intersects search, status and lock", async () => {
    ...
    expect(
      (await run(application.id, { q: "trial", status: "active", lock: "unlocked" })).total,
    ).toBe(1);

    // Same license, but it is not locked, so this must find nothing.
    expect(
      (await run(application.id, { q: "trial", status: "active", lock: "locked" })).total,
    ).toBe(0);
  });

  it("reports total for the filtered set, not the whole application", async () => {
    const application = await makeApplication(db, { ownerId: DEVELOPER_A });
    await seedCast(application.id);
...
describe("queryLicenses — sorting", () => {
  it("sorts newest first by default", async () => {
    ...
    const result = await run(application.id, { sort: "newest" });
  ...
  it("sorts oldest first", async () => {
    ...
    const result = await run(application.id, { sort: "oldest" });
    expect(result.rows[0]?.id).toBe(cast.permanent.id);
  ...
  it("sorts by label alphabetically with unlabeled licenses last", async () => {
    ...
    const result = await run(application.id, { sort: "label" });
  ...
  it("sorts by soonest expiry with permanent licenses last", async () => {
    ...
    const result = await run(application.id, { sort: "expiresSoon" });
    expect(result.rows[0]?.id).toBe(cast.expired.id);
  ...
  it("sorts by most recently seen with never-activated licenses last", async () => {
    ...
    const result = await run(application.id, { sort: "lastSeen" });
...
describe("queryLicenses — pagination", () => {
  async function seedNumbered(applicationId: string, count: number) {
  ...
  it("returns the requested page and a stable page count", async () => {
    const application = await makeApplication(db, { ownerId: DEVELOPER_A });
    await seedNumbered(application.id, 30);

    const first = await run(application.id, { page: 1, pageSize: 25 });
    expect(first.rows).toHaveLength(25);
    expect(first.total).toBe(30);
    expect(first.pageCount).toBe(2);

    const second = await run(application.id, { page: 2, pageSize: 25 });
  ...
  it("orders deterministically when the sort key ties", async () => {
    ...
    for (let index = 0; index < 30; index += 1) {
      await makeLicense(db, { applicationId: application.id, createdAt: sameInstant });
    }

    const firstRun = await run(application.id, { pageSize: 25 });
    const secondRun = await run(application.id, { pageSize: 25 });
    expect(firstRun.rows.map((row) => row.id)).toEqual(
      secondRun.rows.map((row) => row.id),
    );

    const pageOne = await run(application.id, { page: 1, pageSize: 25 });
    const pageTwo = await run(application.id, { page: 2, pageSize: 25 });
  ...
  it("returns an empty page past the end without failing", async () => {
    const application = await makeApplication(db, { ownerId: DEVELOPER_A });
    await seedNumbered(application.id, 3);

    const result = await run(application.id, { page: 9, pageSize: 25 });
    expect(result.rows).toEqual([]);
    expect(result.total).toBe(3);
  });

  it("reports one page for an empty application", async () => {
    const application = await makeApplication(db, { ownerId: DEVELOPER_A });
...
describe("getApplicationLicenseStats", () => {
  it("excludes expired licenses from the active count", async () => {
    // Alpha_v1 counted `status = 'active'`, so an application whose licenses had
    // all lapsed still reported them as active — the one number a developer
    // is most likely to trust at a glance.
    const application = await makeApplication(db, { ownerId: DEVELOPER_A });
    await seedCast(application.id);

    const stats = await getApplicationLicenseStats(db, DEVELOPER_A, application.id, NOW);
  ...
  it("agrees with the filtered queries it summarises", async () => {
    ...
    const stats = await getApplicationLicenseStats(db, DEVELOPER_A, application.id, NOW);
    expect(stats.active).toBe((await run(application.id, { status: "active" })).total);
    expect(stats.expired).toBe((await run(application.id, { status: "expired" })).total);
    expect(stats.revoked).toBe((await run(application.id, { status: "revoked" })).total);
  });

  it("returns zeroes for an application with no licenses", async () => {
    const application = await makeApplication(db, { ownerId: DEVELOPER_A });

    expect(await getApplicationLicenseStats(db, DEVELOPER_A, application.id, NOW)).toEqual({
      total: 0,
      active: 0,
      expired: 0,
      revoked: 0,
  ...
  it("reports no activations for an application nothing has verified against", async () => {
    const application = await makeApplication(db, { ownerId: DEVELOPER_A });
    await makeLicense(db, { applicationId: application.id });

    expect((await getApplicationLicenseStats(db, DEVELOPER_A, application.id, NOW)).activated).toBe(0);
  });
  ...
  it("refuses an application owned by another developer", async () => {
    ...
    await expect(
      getApplicationLicenseStats(db, DEVELOPER_A, application.id, NOW),
    ).rejects.toThrow(/not found/i);
  });
});
