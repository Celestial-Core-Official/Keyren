...
async function countRows(applicationId: string): Promise<number> {
  const rows = await db.select().from(licenses).where(eq(licenses.applicationId, applicationId));
  return rows.length;
}
...
describe("createLicenseBatch — quantity bounds", () => {
  it("creates exactly one license for quantity 1", async () => {
    ...
    const result = await batch(application.id, { quantity: 1 });
    expect(result).toHaveLength(1);
    expect(await countRows(application.id)).toBe(1);
  });

  it("creates exactly 100 licenses for the maximum quantity", async () => {
    const application = await makeApplication(db, { ownerId: DEVELOPER_A });

    const result = await batch(application.id, { quantity: BATCH_QUANTITY_MAX });
    expect(result).toHaveLength(100);
    expect(await countRows(application.id)).toBe(100);
  });

  it("refuses quantity 0", async () => {
    const application = await makeApplication(db, { ownerId: DEVELOPER_A });

    await expect(batch(application.id, { quantity: 0 })).rejects.toThrow();
    expect(await countRows(application.id)).toBe(0);
  });

  it("refuses quantity 101", async () => {
    const application = await makeApplication(db, { ownerId: DEVELOPER_A });

    await expect(batch(application.id, { quantity: 101 })).rejects.toThrow();
    expect(await countRows(application.id)).toBe(0);
  });
  ...
  it("refuses a fractional quantity", async () => {
    ...
    await expect(batch(application.id, { quantity: 2.5 })).rejects.toThrow();
    expect(await countRows(application.id)).toBe(0);
  });
});

describe("createLicenseBatch — validation schema bounds", () => {
  const base = { applicationId: "app_abc", mode: "permanent" as const, hwidLocked: true };
...
describe("createLicenseBatch — labels", () => {
  it("preserves the base label exactly for a single license", async () => {
    ...
    const result = await batch(application.id, { quantity: 1, label: "Acme Corp" });
  ...
  it("suffixes labels with a 1-based index for a batch", async () => {
    ...
    const result = await batch(application.id, { quantity: 3, label: "Acme Corp" });
  ...
  it("leaves labels null when no base label is given", async () => {
    ...
    const result = await batch(application.id, { quantity: 3, label: null });
  ...
  it("keeps a suffixed label inside the 120-character column limit", async () => {
    ...
    const result = await batch(application.id, {
      quantity: 100,
      label: "L".repeat(119),
    });
  ...
  it("applies the same notes to every license in the batch", async () => {
    ...
    await batch(application.id, { quantity: 3, notes: "Bulk order, March." });
    const rows = await db.select().from(licenses).where(eq(licenses.applicationId, application.id));
...
describe("createLicenseBatch — shared settings", () => {
  ...
  it("applies one device-lock setting to the whole batch", async () => {
    ...
    const result = await batch(application.id, { quantity: 4, hwidLocked: false });
...
describe("createLicenseBatch — keys", () => {
  it("returns a distinct plaintext key for every license", async () => {
    ...
    const result = await batch(application.id, { quantity: 50 });
  ...
  it("stores a distinct hash for every license and never the plaintext", async () => {
    ...
    const result = await batch(application.id, { quantity: 10 });
    const rows = await db.select().from(licenses).where(eq(licenses.applicationId, application.id));
  ...
  it("stores a hash that verifies against the returned plaintext", async () => {
    ...
    const result = await batch(application.id, { quantity: 3 });
    const rows = await db.select().from(licenses).where(eq(licenses.applicationId, application.id));
  ...
  it("records the correct last four characters for each key", async () => {
    ...
    const result = await batch(application.id, { quantity: 5 });
    for (const created of result) {
...
describe("createLicenseBatch — atomicity", () => {
  ...
  it("leaves existing licenses untouched when a batch fails", async () => {
    const application = await makeApplication(db, { ownerId: DEVELOPER_A });
    await batch(application.id, { quantity: 2, label: "Existing" });

    const duplicate = generateLicenseKey();
    await expect(
      batch(application.id, { quantity: 5, generateKey: () => duplicate }),
    ).rejects.toThrow();

    expect(await countRows(application.id)).toBe(2);
  });
