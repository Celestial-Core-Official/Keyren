...
- **Handle every error code**, not just the happy path and one generic failure. `LICENSE_REVOKED` and `LICENSE_EXPIRED` are typically worth a specific, actionable message to the end user; `RATE_LIMITED` should back off and retry; `INTERNAL_ERROR` is safe to retry; `LICENSE_INVALID`, `APPLICATION_INVALID`, and `DEVICE_MISMATCH` mean the current key/device combination will not succeed without developer or end-user action and should not be retried in a loop.
